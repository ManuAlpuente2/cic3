...
<header id="rs-header" class="single-header header-style5 mainsmenu<?php echo esc_attr($main_menu_hides);?> <?php echo esc_attr($main_menu_center);?> <?php echo esc_attr($main_menu_icon);?>">
    ...
    <div class="header-inner <?php echo esc_attr($sticky_menu);?>">
        ...
        <div class="menu-area menu_type_<?php echo esc_attr($main_menu_type);?>">
            <div class="<?php echo esc_attr($header_width);?>" <?php echo wp_kses($menu_bg_color, 'konstruk');?>>
                <?php 
                        ...
                        if($rs_show_quote != 'hide'){
                            if(!empty($konstruk_option['quote_btns']) || ($rs_show_quote == 'show') ){ ?>
                                <?php if($quote_btn_texts !=''){?>
                                    <div class="btn_quote">
                                        <a href="<?php echo esc_url($konstruk_option['quote_link']); ?>" class="quote-button"><?php  echo esc_html($quote_btn_texts); ?>                                            
                                        </a>
                                    </div>
                                        
                                    <?php } else {
                                        if(!empty($konstruk_option['quote'])){?>
                                        <div class="btn_quote">
                                            <a href="<?php echo esc_url($konstruk_option['quote_link']); ?>" class="quote-button"><?php  echo esc_html($konstruk_option['quote']); ?>                                            
                                            </a>
                                        </div>
                                    <?php } }
                                ?>  
